  ...
  namespace lj {
    ...
    public:
      ...
      void read() {
        ...
        while (!buf.index_better_size()) {
          ...
          if (!len) throw std::runtime_error("LuaJIT: Prototype size == 0.");

          proto pt = {0};
          read_proto(pt);
          if (pt.len != len) throw std::runtime_error("LuaJIT: Different prototype sizes.");
          temp_protos.push_back(pt);
          protos.push_back(pt);
        }

        compiler = COMPILER_LUAJIT;
      }
    ...
    private:
      void read_header() {
        ...
        byte ver = buf.read();
        if (ver > 2 || ver < 1) throw std::runtime_error("LuaJIT: Unknown version.");
        switch (ver) {
        case 1: version = CV_LUAJIT1; break;
        case 2: version = CV_LUAJIT2; break;
        default: break;
        }
        ...
        flags &= DUMP_STRIP;
        flags &= DUMP_FFI;
        if (version == CV_LUAJIT2) flags &= DUMP_FR2;
        if (flags) throw std::runtime_error("LuaJIT: Unknown header flags.");
      ...
      void read_proto(proto &pt) {
        ...
        flags &= PROTO_NOJIT;
        flags &= PROTO_ILOOP;
        if (flags) throw std::runtime_error("LuaJIT: Unknown prototype flags.");
        ...
        if (sizedbg_read != sizedbg) throw std::runtime_error("LuaJIT: Invalid debuginfo size.");
      ...
      void write_lineinfo(proto &pt, buffer &ptdebug) {
        if (pt.lineinfo.size() != pt.ins.size()) throw std::runtime_error("LuaJIT: Lines count != instructions count");
      ...
      void write_uvname(proto &pt, buffer &ptdebug) {
        if (pt.lineinfo.size() != pt.ins.size()) throw std::runtime_error("LuaJIT: Count of upvalue names != size of BC instructions");
